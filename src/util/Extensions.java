...
public class Extensions {
    ...
    public static <T> List<T> override(List<T> initial, List<T> newList) {
        initial.clear();
        initial.addAll(newList);
        return initial;
    }

    public static <T> Set<Set<T>> generateSubsets(Stream<T> values, int size) {
        return Util.generateSubsets(size, values);
    }
}